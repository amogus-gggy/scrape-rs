...
mod tests {
    ...
    fn invalid_selector_is_empty_not_panic() {
        ...
        assert_eq!(doc.first("::::").map(|n| n.text()), None);
        assert!(select_all(PAGE, "::::").is_empty());
        assert_eq!(select_first(PAGE, "::::"), None);
        assert!(select_html(PAGE, "::::").is_empty());
    }

    #[test]
    fn free_functions_keep_previous_behaviour() {
        assert_eq!(select_first(PAGE, ".text").as_deref(), Some("Aaa"));
        assert_eq!(select_all(PAGE, ".text"), vec!["Aaa", "Bbb"]);
        assert_eq!(select_html(PAGE, ".quote").len(), 2);
        // repeated calls must hit the selector cache and stay correct
        assert_eq!(select_all(PAGE, ".text"), vec!["Aaa", "Bbb"]);
    }
}
